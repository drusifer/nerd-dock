...
def parse_skill_md(skill_md_path: Path) -> tuple[str, str, list[str], list[str], str]:
    """Parse SKILL.md file to extract name, description, triggers, requires, and the remaining system prompt."""
    ...
    if content.startswith("---"):
        ...
        if len(parts) >= 3:
            try:
                ...
                triggers = get_list(metadata.get("triggers"))
                requires = get_list(metadata.get("requires"))
                system_prompt = parts[2].strip()
            except Exception as e:
                print(f"  ⚠️  Failed to parse YAML frontmatter for {skill_md_path}: {e}")
                
    if not name:
        name = skill_md_path.parent.name.replace(".docs", "")
    if not description:
        description = f"{name.capitalize()} Persona"
        
    return name, description, triggers, requires, system_prompt


def setup_antigravity_agents(project_root: Path, personas: list, shared_skills: list) -> int:
    """Create .agents/ structure with symlinks for skills and agent.json files for agents."""
    print("\n📁 Setting up Antigravity Agents and Skills (.agents/)...")
    ...
    # 1. Setup shared skills in .agents/skills/
    for skill_name, skill_dir in shared_skills:
        skill_link = skills_dir / skill_name
        if create_symlink(skill_link, skill_dir):
            print(f"  ✅ .agents/skills/{skill_name} -> {skill_dir.relative_to(project_root)}")
            count += 1

    # 2. Setup persona skills in .agents/skills/
    for persona_name, persona_dir in personas:
        skill_link = skills_dir / persona_name
        if create_symlink(skill_link, persona_dir):
            print(f"  ✅ .agents/skills/{persona_name} -> {persona_dir.relative_to(project_root)}")
            count += 1

    # 3. Setup personas as agents in .agents/agents/
    for persona_name, persona_dir in personas:
        ...
        try:
            name, description, triggers, requires, system_prompt = parse_skill_md(skill_md)
            
            # Create the agent directory
            agent_folder = agents_dir / persona_name
            agent_folder.mkdir(parents=True, exist_ok=True)
            
            agent_json_path = agent_folder / "agent.json"
            ...
            # Write agent.json cleanly
            agent_json_path.write_text(json.dumps(agent_data, indent=2) + "\n")
            print(f"  ✅ Created Antigravity Agent Card: .agents/agents/{persona_name}/agent.json")
            count += 1
        except Exception as e:
            print(f"  ⚠️  Failed to create agent.json for {persona_name}: {e}")
...
def main():
    ...
    total += setup_antigravity_agents(project_root, personas, shared_skills)
    ...
    print(f"\n✅ Done! Created/Updated {total} symlinks and agent configs.")
    print("\nAgent discovery is now enabled for:")
    print("  • Antigravity / agy (.agents/)")
