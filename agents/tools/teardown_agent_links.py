...
def remove_antigravity_agents(project_root: Path, dry_run: bool) -> int:
    """Remove generated Antigravity agent directories."""
    agents_dir = project_root / ".agents" / "agents"
    print(f"\n📁 Tearing down Antigravity Agents ({agents_dir})...")
    ...
    for agent_folder in sorted(agents_dir.iterdir()):
        if agent_folder.is_dir():
            ...
            if agent_json.exists():
                if dry_run:
                    print(f"  DRY-RUN remove folder {agent_folder}")
                    count += 1
                else:
                    try:
                        shutil.rmtree(agent_folder)
                        print(f"  ✅ Removed agent folder {agent_folder.relative_to(project_root)}")
                        count += 1
                    except OSError as exc:
                        print(f"  ⚠️  Could not remove folder {agent_folder}: {exc}")

    remove_empty_dir(agents_dir, dry_run)
    remove_empty_dir(project_root / ".agents", dry_run)
    return count
...
def main():
    ...
    total += remove_skill_links(project_root / ".agents" / "skills", project_root, "Antigravity Skills", args.dry_run)
    total += remove_antigravity_agents(project_root, args.dry_run)
